    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.bucket = s3.Bucket(
            ...
            enforce_ssl=True,
            # Production retains the bucket: it holds the assets a running SPA is
            # still loading, and CloudFront keeps serving a cached index.html
            # that references them for as long as its TTL says to.
            #
            # Staging does not. Its contents are `npm run build` output,
            # regenerated by every deploy from the commit being deployed, and a
            # retained bucket after `cdk destroy` is orphaned storage nobody will
            # remember to empty. `auto_delete_objects` is required for DESTROY to
            # do anything — S3 refuses to delete a bucket that still has objects,
            # and CloudFormation would leave the stack in DELETE_FAILED.
